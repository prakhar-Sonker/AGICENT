//check a number is 3 digit number or not
// a = 45
// if(a>=100 && a<=999) console.log('It is a 3 digit number')
// else console.log('Not 3 digit number') 


//js me age == use krte hai to wo automatically convert krdeta hai agr string hai to int me naki java ki trh fxn likho parseToInt() 
a=5
b='5'
//this will give o/p true bcz wo uska type nhi check kr rha hai sirf wo value dekh rha hai --> to solve this problem triple === is there
console.log(a==b)

// what is === tripleequalto 
//now if we do a===b it will give false bcz ab wo uska type check krke value dega ki dono same hai ya nhi 
console.log(a===b)


